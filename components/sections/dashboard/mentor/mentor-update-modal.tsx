...
interface MentorUpdateModalProps {
  isOpen: boolean
  onClose: () => void
  mentor?: any
}

export function MentorUpdateModal({ isOpen, onClose, mentor }: MentorUpdateModalProps) {
  ...
  useEffect(() => {
    if (mentor && isOpen) {
      setFormData({
        bio: mentor.bio || "",
        dailyRate: mentor.daily_rate.toString() || "",
        weeklyRate: mentor.weekly_rate.toString() || "",
        monthlyRate: mentor.monthly_rate.toString() || "",
        weekdays: mentor.availability?.weekdays || [],
        hours: mentor.availability?.hours || "",
      })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.type === updateMentorProfile.fulfilled.type) {
      onClose()
    }
  }
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <Card className="w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-background border-b border-border/50 p-6 flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold text-foreground">Update Mentor Profile</h2>
            <p className="text-sm text-muted-foreground mt-1">Modify your mentorship details</p>
          </div>
